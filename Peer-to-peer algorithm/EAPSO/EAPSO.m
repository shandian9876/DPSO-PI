...
function [BestPosition,BestValue,diversity,everyfit] =EAPSO(fhd,nPop,nVar,VarMin,VarMax,MaxIt,X,varargin)
...
GlobalBest.Cost=inf;
everyfit=zeros(1,MaxIt);
for i=1:nPop

    particle(i).Position=X(i,:);
    particle(i).Velocity=zeros(VarSize);
%     particle(i).Cost= fhd(particle(i).Position);
    particle(i).Cost = feval(fhd,particle(i).Position',varargin{:});
%     
%     fprintf("第一个粒子的适应值=%d\n",particle(i).Cost);
...
for i=1:nPop
    ...
    diversity(1)=sum(sqrt(sum((X-mean(X)).^2,2)))/nPop;
...
for it=2:MaxIt
    ...
    diversity(it)=sum(sqrt(sum((X-mean(X)).^2,2)))/nPop;
    ...
    for i=1:nPop/2
        ...
        X(LOSER(i),:)=particle(LOSER(i)).Position;
%         particle(LOSER(i)).Cost = fhd(particle(LOSER(i)).Position);
        particle(LOSER(i)).Cost = feval(fhd,particle(i).Position',varargin{:});
