...
name21='Goldstein Price'; name22='Six-hump camel back';name23='Pathological';name24='Schewfel_1.2';name25='Quadric_Noise';
...
% 初始化保存结果的几个矩阵
CLPSO_new_fitcount_res=zeros(length(funchoose),runtimes);        % 初始化为 0
CLPSO_new_gbestval_res=ones(length(funchoose),runtimes)*inf;     % 初始化为无穷
CLPSO_new_gbest_res=ones(length(funchoose),runtimes,D)*inf;      % 初始化为无穷
bestresults=ones(runtimes,length(funchoose))*inf;

gbestval_res_1=ones(length(funchoose),runtimes)*inf;    
bestresults_1=ones(runtimes,length(funchoose))*inf;     
 
gbestval_res_2=ones(length(funchoose),runtimes)*inf;     
bestresults_2=ones(runtimes,length(funchoose))*inf; 

for funnum=1:length(funchoose)
    fun=funchoose(funnum);
    
    if fun == 76      % frequency_modulated
       % Max_FES=150000;
        D=6; maxgen=ceil(Max_FES/N);  % 最大进化代数
        VRmin(fun)=-6.4; VRmax(fun)=6.35;
    elseif fun == 30  % Gear_Train
      %  Max_FES=150000;
        D=4; maxgen=ceil(Max_FES/N);  % 最大进化代数
        VRmin(fun)=12; VRmax(fun)=60;
    elseif fun == 81  % sprd_spectrum_rad_pphase
       % Max_FES=150000;
        D=20; maxgen=ceil(Max_FES/N);
        VRmin(fun)=0; VRmax(fun)=2*pi;
    elseif fun == 82  % lennard_jones_potential_problem
      %  Max_FES=150000;
...
%             gbias=0.8.*(VRmin(fun)+(VRmax(fun)-VRmin(fun)).*rand(1,D));           
            gbias=VRmin(fun)+(VRmax(fun)-VRmin(fun).*rand(1,D));
...
%         N=20;  % TRAPSO
        ...
        [CLPSO_new_gbest,CLPSO_new_gbestval,CLPSO_new_fitcount,suc, suc_fes]= TAPSO_func(jingdu,fhd,maxgen,Max_FES,N,D,VRminn(fun),VRmaxn(fun),fun,VRmin(fun),VRmax(fun),gbias,norm_flag,shift_flag); 
